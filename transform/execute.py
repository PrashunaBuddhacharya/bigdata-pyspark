...
def load_and_clean(spark, input_dir,output_dir ):
    ...
    recommendations_df = spark.read.schema(recommendations_schema).json(os.path.join(input_dir, "dict_artists_fixed.json"))
    ...
    print("Stage 1: Data loaded and cleaned successfully.")
...
def create_master_table(artists_df, recommendations_df, tracks_df, output_dir):
    ...
    print(f"After exploding tracks: {tracks_exploded.count()} rows")
    ...
    print(f"After joining with artists: {master_df.count()} rows")
    ...
    print("Unmatched artist IDs (missing from artists data):")
    ...
    print(f"Final master table rows: {final_df.count()}")

    # Show sample data
    print("Sample master table data:")
    final_df.show(5)
...
if __name__ == "__main__":
    ...
    # ADD DEBUG HERE
    print("=== DEBUGGING BEFORE MASTER TABLE CREATION ===")
    print(f"Artists count: {artists_df.count()}")
    print(f"Tracks count: {tracks_df.count()}")
